...
class Cache
{
     ...
     * entry holds an absolute path that reaches inside `phpstan.phar`, and the `phar` belongs to
     * the project that wrote the entry. A second project then reads a path that it does not own.
     * The failure appears when the first project moves or goes away: PHPStan reports that a file
     * inside a `phar` "is not a file", and it names a directory that belongs to another
     * repository. A git worktree makes this common, because a developer removes a worktree when
     * the work ends.
     *
